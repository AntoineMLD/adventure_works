...
# Décompresser le fichier ZIP dans le répertoire "unzip_folder"
unzip downloaded_blobs/machine_learning/reviews.zip -d unzip_folder

# Vérifier si le fichier "amazon_review_polarity_csv.tgz" existe dans le répertoire
if [ -f "unzip_folder/amazon_review_polarity_csv.tgz" ]; then
    ...
    # Supprimer le fichier .tgz après décompression
...
# Renommer et déplacer les fichiers test.csv et train.csv du dossier unzip_folder
...
# Renommer et déplacer les fichiers test.csv et train.csv du dossier amazon_review_polarity_csv
